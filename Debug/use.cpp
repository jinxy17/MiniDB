...
int main(int argc, char **argv) {
	...
	DataOperater *qlm = new DataOperater(smm, ixm, fileManager, bufPageManager);
	...
	while (1) {
		...
		if (cur == "exit") {
			...
			if (cur == "table") {
				...
				// 只处理单列索引
				if(attrs.size() == 0)
					smm->DropIndex(tableName, attrs[0]);
			}
		} 
		else if (cur == "insert") {
			cur = readIdentifier();
			...
			if (cur == "values") {
				...
				cout<<col<<" "<<nullnub<<endl;
				qlm->Insert(tableName, datas, cols, nullnub);
			}
		}
		else if (cur == "update") {
			string tableName = readIdentifier();
			...
			if (cur == "set") {
				...
				// 	assign.assignnull = false;
				// 	if(value == "null") {
				// 		assign.assignnull = true;
				...
				while(1) {
					...
					if (c == '=') {
						int type;
						string value = readValue(&type);
						assigns.attrs.push_back(attr);
						if(value == "null") {
							assigns.assignnull.push_back(true);
							assigns.values.push_back(nullptr);
						}else if (type == INT) {
							int *d = new int; *d = atoi(value.c_str());
							assigns.assignnull.push_back(false);
							assigns.values.push_back((BufType)d);
						} else if (type == FLOAT) {
							double *d = new double; *d = atof(value.c_str());
							assigns.assignnull.push_back(false);
							assigns.values.push_back((BufType)d);
